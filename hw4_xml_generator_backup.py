...
def create_tree_xml():
    ...
    z_left = np.random.uniform(-1.25, -0.75)
    z_right = np.random.uniform(-0.75, -0.25)

    leaf_count = 1
    for i in range(1, 5):
        if i == 2:  # np.randomize z positions for the second set of branches
            z_left = np.random.uniform(0.3, 0.7)
            z_right = np.random.uniform(0.3, 0.7)
        ...
        # Increment z positions for the next set of branches
        if i > 2:
            z_left += 0.75
            z_right += 0.75
